...
## ChoiceList
Purpose: Group of radio buttons or checkboxes. Better than raw checkboxes for mutually exclusive or grouped options.
When to use: Selecting one (allowMultiple: false) or more (allowMultiple: true) from a defined set of options. Includes title and help text per choice.
...
When to use: Any user action. Use primary for the most important action on a page or section. Use plain for lower-emphasis actions in dense contexts.
When NOT to use: Don't use more than one primary button per section. Don't use buttons for navigation — use Link.
Key props: variant (primary/secondary/tertiary/plain/destructive), size (slim/medium/large), disabled, loading, url (renders as anchor), fullWidth, icon.
...
## Banner
Purpose: Persistent contextual messaging — for system states, warnings, errors, or informational content that the user needs to notice.
When to use: Status messages that are tied to a page or section, errors from form submission, upgrade prompts, warnings about state.
When NOT to use: Don't use Banner for transient feedback after an action (use Toast). Don't use for validation errors on individual fields (use TextField error prop).
...
## Modal
Purpose: Focused overlay for tasks that require user input or confirmation before continuing. Interrupts the current flow.
When to use: Confirmations (delete, irreversible action), focused data entry (add address, quick edit), alerts requiring acknowledgment.
When NOT to use: Don't use for navigation. Don't nest modals. Don't use for complex multi-step flows — use a page or sheet instead.
Key props: open, onClose, title (required), primaryAction, secondaryActions, size (small/medium/large/fullscreen), sectioned.
Common scenarios: Delete confirmation ("Are you sure?"), bulk action confirmation, quick add form, image preview.
...
Common scenarios: Product status (Active/Draft), order fulfillment status, plan tier labels, notification counts.
Doc URL: https://polaris.shopify.com/components/feedback-indicators/badge

## EmptyState
Purpose: Structured placeholder for when a list, table, or view has no content. Guides users toward populating it.
When to use: First-time use of a feature (no products yet), filtered list with no results, no-permission state, error loading state.
Key props: heading, image/illustration, action (primary CTA), secondaryAction, children (body text).
Common scenarios: "No products yet — add your first product," "No results for this filter," "You don't have access to this feature."
Doc URL: https://polaris.shopify.com/components/layout-and-structure/empty-state

## Filters
Purpose: Filter bar for IndexTable and ResourceList. Provides a standard, accessible filter UI with query search, filter chips, and saved views.
When to use: Any list/table that users need to filter by multiple criteria (status, date range, tags, etc.)
When NOT to use: Don't build custom filter UI when Filters covers the pattern. Don't use for simple single-value filtering — a Select may suffice.
Key props: filters (filter definitions), appliedFilters, onQueryChange, onClearAll, queryValue.
Common scenarios: Product list filters (status, vendor, type), order filters (fulfillment, payment, date), customer filters.
...
## Tabs
Purpose: Segment content within a page section. Switches between related views without a page navigation.
When to use: When content can be logically divided into 2–5 views that share the same context (e.g., All / Active / Draft / Archived).
When NOT to use: Don't use tabs for different pages — use navigation. Don't use more than 5–6 tabs; consider a Select or segmented content structure.
Key props: tabs, selected, onSelect, fitted (equal-width tabs).
Common scenarios: Product list status tabs (All/Active/Draft), order view tabs (Summary/Timeline/Shipping), settings tab navigation.
...
When to use: Confirming that a user action succeeded (saved, copied, deleted). Feedback that doesn't require the user to act.
When NOT to use: Don't use for errors that block workflow — use Banner or inline errors. Don't use for messages requiring a response.
...
When NOT to use: Don't use for complex forms or content that requires a lot of vertical space — use Modal or Sheet.
Key props: Popover: active, activator, onClose. ActionList: items (each with content, onAction, destructive, disabled).
Common scenarios: Row overflow menu in IndexTable, "More actions" button, bulk action menu, split button actions.
Doc URL: https://polaris.shopify.com/components/overlays/popover

## Pagination
Purpose: Controls for navigating through paginated content. Used below IndexTable or ResourceList.
